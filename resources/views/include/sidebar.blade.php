<div class="sidebar-wrapper sidebar-theme">

    <nav id="sidebar">
        <div class="shadow-bottom"></div>
        <ul class="list-unstyled menu-categories ps ps--active-y" id="accordionExample">
            <li class="menu">
                <a href="{{ route('backend.admin.dashboard') }}" aria-expanded="false" class="dropdown-toggle"
                   @if(route_exist_in_sidebar(['backend.admin.dashboard'])) data-active="true" @endif>
                    <div class="">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                             stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                             class="feather feather-home">
                            <path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"></path>
                            <polyline points="9 22 9 12 15 12 15 22"></polyline>
                        </svg>
                        <span>Dashboard</span>
                    </div>
                </a>
            </li>

            <li class="menu">
                @php
                    $catalogueRoutes = [
                        'backend.service.index',
                        'backend.service.create',
                        'backend.service.edit',
                    ];
                @endphp
                <a href="#frontServiceMenu" data-toggle="collapse"
                   aria-expanded="@if(route_exist_in_sidebar($catalogueRoutes) && request()->get('service_type') =='long') true @else false @endif"
                   @if(route_exist_in_sidebar($catalogueRoutes)) data-active="true" @endif
                   class="dropdown-toggle @if(!route_exist_in_sidebar($catalogueRoutes))collapsed @endif">
                    <div class="">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                             stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                             class="feather feather-menu">
                            <line x1="3" y1="12" x2="21" y2="12"></line>
                            <line x1="3" y1="6" x2="21" y2="6"></line>
                            <line x1="3" y1="18" x2="21" y2="18"></line>
                        </svg>
                        <span>Long Service</span>
                    </div>
                    <div>
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                             stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                             class="feather feather-chevron-right">
                            <polyline points="9 18 15 12 9 6"></polyline>
                        </svg>
                    </div>
                </a>
                <ul class="collapse submenu list-unstyled @if(route_exist_in_sidebar($catalogueRoutes) && request()->get('service_type') =='long') show @endif"
                    id="frontServiceMenu" data-parent="#accordionExample">


                    <li><a href="{{ route('backend.service.create',['service_type'=>'long']) }}">Add+</a>
                    </li>
                    <li><a href="{{ route('backend.service.index',['service_type'=>'long']) }}">List</a>
                    </li>

                </ul>
            </li>
            <li class="menu">
                @php
                    $catalogueRoutes = [
                        'backend.service.index',
                        'backend.service.create',
                        'backend.service.edit',
                    ];
                @endphp
                <a href="#onDemandServiceMenu" data-toggle="collapse"
                   aria-expanded="@if(route_exist_in_sidebar($catalogueRoutes) && request()->get('service_type') =='short') true @else false @endif"
                   @if(route_exist_in_sidebar($catalogueRoutes)) data-active="true" @endif
                   class="dropdown-toggle @if(!route_exist_in_sidebar($catalogueRoutes))collapsed @endif">
                    <div class="">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                             stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                             class="feather feather-menu">
                            <line x1="3" y1="12" x2="21" y2="12"></line>
                            <line x1="3" y1="6" x2="21" y2="6"></line>
                            <line x1="3" y1="18" x2="21" y2="18"></line>
                        </svg>
                        <span>On Demand Service</span>
                    </div>
                    <div>
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                             stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                             class="feather feather-chevron-right">
                            <polyline points="9 18 15 12 9 6"></polyline>
                        </svg>
                    </div>
                </a>
                <ul class="collapse submenu list-unstyled @if(route_exist_in_sidebar($catalogueRoutes) && request()->get('service_type') =='short') show @endif"
                    id="onDemandServiceMenu" data-parent="#accordionExample">


                    <li><a href="{{ route('backend.service.create',['service_type'=>'short']) }}">Add+</a>
                    </li>
                    <li><a href="{{ route('backend.service.index',['service_type'=>'short']) }}">List</a>
                    </li>

                </ul>
            </li>

            <li class="menu">
                @php
                    $orderBackendRoutes = [
                        'backend.order.index',
                        'backend.order.edit',
                        'backend.order.show',
                        'backend.order.edit',

                    ];
                @endphp
                <a href="#orderBackendMenu" data-toggle="collapse"
                   aria-expanded="@if(route_exist_in_sidebar($orderBackendRoutes)) true @else false @endif"
                   @if(route_exist_in_sidebar($orderBackendRoutes)) data-active="true" @endif
                   class="dropdown-toggle @if(!route_exist_in_sidebar($orderBackendRoutes))collapsed @endif">
                    <div class="">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-mouse-pointer"><path d="M3 3l7.07 16.97 2.51-7.39 7.39-2.51L3 3z"></path><path d="M13 13l6 6"></path></svg>
                        <span>Order List</span>
                    </div>
                    <div>
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                             stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                             class="feather feather-chevron-right">
                            <polyline points="9 18 15 12 9 6"></polyline>
                        </svg>
                    </div>
                </a>
                <ul class="collapse submenu list-unstyled @if(route_exist_in_sidebar($orderBackendRoutes)) show @endif"
                    id="orderBackendMenu" data-parent="#accordionExample">


                    <li>
                        <a href="{{ route('backend.order.index',) }}">Order List</a>
                    </li>

                </ul>
            </li>

            <li class="menu">
                @php
                    $catalogueRoutes = [
                        'backend.long-term-service.service-request.create',
                        'backend.long-term-service.service-request.index',
                        'backend.long-term-service.service-request.edit',

                    ];
                @endphp
                <a href="#longTermServiceProcess" data-toggle="collapse"
                   aria-expanded="@if(route_exist_in_sidebar($catalogueRoutes)) true @else false @endif"
                   @if(route_exist_in_sidebar($catalogueRoutes)) data-active="true" @endif
                   class="dropdown-toggle @if(!route_exist_in_sidebar($catalogueRoutes))collapsed @endif">
                    <div class="">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                             stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                             class="feather feather-menu">
                            <line x1="3" y1="12" x2="21" y2="12"></line>
                            <line x1="3" y1="6" x2="21" y2="6"></line>
                            <line x1="3" y1="18" x2="21" y2="18"></line>
                        </svg>
                        <span>Long Service Process</span>
                    </div>
                    <div>
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                             stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                             class="feather feather-chevron-right">
                            <polyline points="9 18 15 12 9 6"></polyline>
                        </svg>
                    </div>
                </a>
                <ul class="collapse submenu list-unstyled @if(route_exist_in_sidebar($catalogueRoutes)) show @endif"
                    id="longTermServiceProcess" data-parent="#accordionExample">


                    <li>
                        <a href="{{ route('backend.long-term-service.service-request.index',['service_type'=>'long']) }}">Service
                            Request</a>
                    </li>

                </ul>
            </li>


            <li class="menu">
                @php
                    $couponRoutes = [
                        'backend.coupon.create',
                        'backend.coupon.edit',
                        'backend.coupon.update',
                        'backend.coupon.index',
                    ];
                @endphp
                <a href="#couponMenu" data-toggle="collapse"
                   aria-expanded="@if(route_exist_in_sidebar($couponRoutes)) true @else false @endif"
                   @if(route_exist_in_sidebar($couponRoutes)) data-active="true" @endif
                   class="dropdown-toggle @if(!route_exist_in_sidebar($couponRoutes))collapsed @endif">
                    <div class="">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                             stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                             class="feather feather-aperture">
                            <circle cx="12" cy="12" r="10"></circle>
                            <line x1="14.31" y1="8" x2="20.05" y2="17.94"></line>
                            <line x1="9.69" y1="8" x2="21.17" y2="8"></line>
                            <line x1="7.38" y1="12" x2="13.12" y2="2.06"></line>
                            <line x1="9.69" y1="16" x2="3.95" y2="6.06"></line>
                            <line x1="14.31" y1="16" x2="2.83" y2="16"></line>
                            <line x1="16.62" y1="12" x2="10.88" y2="21.94"></line>
                        </svg>
                        <span>Coupon</span>
                    </div>
                    <div>
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                             stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                             class="feather feather-chevron-right">
                            <polyline points="9 18 15 12 9 6"></polyline>
                        </svg>
                    </div>
                </a>
                <ul class="collapse submenu list-unstyled @if(route_exist_in_sidebar($couponRoutes)) show @endif"
                    id="couponMenu" data-parent="#accordionExample">

                    <li>
                        <a href="{{ route('backend.coupon.create') }}">Create Coupon</a>
                        <a href="{{ route('backend.coupon.index') }}">Coupon List</a>
                    </li>

                </ul>
            </li>

            <li class="menu">
                @php
                    $appSliderMenu = [
                        'backend.app-slider.create',
                        'backend.app-slider.edit',
                        'backend.app-slider.index',
                    ];
                @endphp
                <a href="#appSliderMenu" data-toggle="collapse"
                   aria-expanded="@if(route_exist_in_sidebar($appSliderMenu)) true @else false @endif"
                   @if(route_exist_in_sidebar($appSliderMenu)) data-active="true" @endif
                   class="dropdown-toggle @if(!route_exist_in_sidebar($appSliderMenu))collapsed @endif">
                    <div class="">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                             stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                             class="feather feather-aperture">
                            <circle cx="12" cy="12" r="10"></circle>
                            <line x1="14.31" y1="8" x2="20.05" y2="17.94"></line>
                            <line x1="9.69" y1="8" x2="21.17" y2="8"></line>
                            <line x1="7.38" y1="12" x2="13.12" y2="2.06"></line>
                            <line x1="9.69" y1="16" x2="3.95" y2="6.06"></line>
                            <line x1="14.31" y1="16" x2="2.83" y2="16"></line>
                            <line x1="16.62" y1="12" x2="10.88" y2="21.94"></line>
                        </svg>
                        <span>App Slider</span>
                    </div>
                    <div>
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                             stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                             class="feather feather-chevron-right">
                            <polyline points="9 18 15 12 9 6"></polyline>
                        </svg>
                    </div>
                </a>
                <ul class="collapse submenu list-unstyled @if(route_exist_in_sidebar($appSliderMenu)) show @endif"
                    id="appSliderMenu" data-parent="#accordionExample">

                    <li><a href="{{ route('backend.app-slider.create') }}">Create App Slider</a>
                    <li>
                    <li><a href="{{ route('backend.app-slider.index') }}">App Slider List</a>
                    <li>

                </ul>
            </li>

            <li class="menu">
                @php
                    $testimonialMenu = [
                        'backend.testimonial.create',
                        'backend.testimonial.edit',
                        'backend.testimonial.index',
                    ];
                @endphp
                <a href="#testimonialMenu" data-toggle="collapse"
                   aria-expanded="@if(route_exist_in_sidebar($testimonialMenu)) true @else false @endif"
                   @if(route_exist_in_sidebar($testimonialMenu)) data-active="true" @endif
                   class="dropdown-toggle @if(!route_exist_in_sidebar($testimonialMenu))collapsed @endif">
                    <div class="">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                             stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                             class="feather feather-aperture">
                            <circle cx="12" cy="12" r="10"></circle>
                            <line x1="14.31" y1="8" x2="20.05" y2="17.94"></line>
                            <line x1="9.69" y1="8" x2="21.17" y2="8"></line>
                            <line x1="7.38" y1="12" x2="13.12" y2="2.06"></line>
                            <line x1="9.69" y1="16" x2="3.95" y2="6.06"></line>
                            <line x1="14.31" y1="16" x2="2.83" y2="16"></line>
                            <line x1="16.62" y1="12" x2="10.88" y2="21.94"></line>
                        </svg>
                        <span>Testimonial</span>
                    </div>
                    <div>
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                             stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                             class="feather feather-chevron-right">
                            <polyline points="9 18 15 12 9 6"></polyline>
                        </svg>
                    </div>
                </a>
                <ul class="collapse submenu list-unstyled @if(route_exist_in_sidebar($testimonialMenu)) show @endif"
                    id="testimonialMenu" data-parent="#accordionExample">

                    <li><a href="{{ route('backend.testimonial.create') }}">Create Testimonial</a>
                    <li>
                    <li><a href="{{ route('backend.testimonial.index') }}">Testimonial List</a>
                    <li>

                </ul>
            </li>

            <li class="menu">
                @php
                    $webFrontServiceMenu = [
                        'backend.front-service.create',
                        'backend.front-service.edit',
                        'backend.front-service.index',
                    ];
                @endphp
                <a href="#webFrontServiceMenu" data-toggle="collapse"
                   aria-expanded="@if(route_exist_in_sidebar($webFrontServiceMenu)) true @else false @endif"
                   @if(route_exist_in_sidebar($webFrontServiceMenu)) data-active="true" @endif
                   class="dropdown-toggle @if(!route_exist_in_sidebar($webFrontServiceMenu))collapsed @endif">
                    <div class="">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                             stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                             class="feather feather-aperture">
                            <circle cx="12" cy="12" r="10"></circle>
                            <line x1="14.31" y1="8" x2="20.05" y2="17.94"></line>
                            <line x1="9.69" y1="8" x2="21.17" y2="8"></line>
                            <line x1="7.38" y1="12" x2="13.12" y2="2.06"></line>
                            <line x1="9.69" y1="16" x2="3.95" y2="6.06"></line>
                            <line x1="14.31" y1="16" x2="2.83" y2="16"></line>
                            <line x1="16.62" y1="12" x2="10.88" y2="21.94"></line>
                        </svg>
                        <span>Front Service</span>
                    </div>
                    <div>
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                             stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                             class="feather feather-chevron-right">
                            <polyline points="9 18 15 12 9 6"></polyline>
                        </svg>
                    </div>
                </a>
                <ul class="collapse submenu list-unstyled @if(route_exist_in_sidebar($webFrontServiceMenu)) show @endif"
                    id="webFrontServiceMenu" data-parent="#accordionExample">

                    <li><a href="{{ route('backend.front-service.create') }}">Create Front Service</a>
                    <li>
                    <li><a href="{{ route('backend.front-service.index') }}">Front Service List</a>
                    <li>

                </ul>
            </li>

            <li class="menu">
                @php
                    $customerRoutes = [
                        'backend.user.create',
                        'backend.user.edit',
                        'backend.user.store',
                        'backend.user.index',
                        'backend.user.view-provider',
                    ];
                @endphp
                <a href="#customerMenu" data-toggle="collapse"
                   aria-expanded="@if(route_exist_in_sidebar($customerRoutes)) true @else false @endif"
                   @if(route_exist_in_sidebar($customerRoutes)) data-active="true" @endif
                   class="dropdown-toggle @if(!route_exist_in_sidebar($customerRoutes))collapsed @endif">
                    <div class="">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                             stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                             class="feather feather-users">
                            <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path>
                            <circle cx="9" cy="7" r="4"></circle>
                            <path d="M23 21v-2a4 4 0 0 0-3-3.87"></path>
                            <path d="M16 3.13a4 4 0 0 1 0 7.75"></path>
                        </svg>
                        <span>User</span>
                    </div>
                    <div>
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                             stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                             class="feather feather-chevron-right">
                            <polyline points="9 18 15 12 9 6"></polyline>
                        </svg>
                    </div>
                </a>
                <ul class="collapse submenu list-unstyled @if(route_exist_in_sidebar($customerRoutes)) show @endif"
                    id="customerMenu" data-parent="#accordionExample">

                    <li>
                        <a href="{{ route('backend.user.index',['role' => 3]) }}">Provider</a>
                        <a href="{{ route('backend.user.index',['role' => 4]) }}">Seeker</a>
                    </li>

                </ul>
            </li>

            @php
                $settingsRoutes = [
                   'backend.specialities.index',
                   'backend.service.service-category.index',
                   'backend.expertise.index',
                ];
            @endphp

            <li class="menu">
                <a href="#app" data-toggle="collapse" aria-expanded="@if(route_exist_in_sidebar($settingsRoutes)) true @else false @endif" class="dropdown-toggle @if(!route_exist_in_sidebar($settingsRoutes)) collapsed @endif">
                    <div class="">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                             stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                             class="feather feather-settings">
                            <circle cx="12" cy="12" r="3"></circle>
                            <path
                                d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-2 2 2 2 0 0 1-2-2v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83 0 2 2 0 0 1 0-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1-2-2 2 2 0 0 1 2-2h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 0-2.83 2 2 0 0 1 2.83 0l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 2-2 2 2 0 0 1 2 2v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 0 2 2 0 0 1 0 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 2 2 2 2 0 0 1-2 2h-.09a1.65 1.65 0 0 0-1.51 1z"></path>
                        </svg>
                        <span>Setting</span>
                    </div>
                    <div>
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                             stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                             class="feather feather-chevron-right">
                            <polyline points="9 18 15 12 9 6"></polyline>
                        </svg>
                    </div>
                </a>
                <ul class="submenu list-unstyled collapse @if(route_exist_in_sidebar($settingsRoutes)) show @endif" id="app" data-parent="#accordionExample" style="">

                    <li>
                        <a href="{{ route('backend.specialities.index') }}"> Specialities </a>
                    </li>
                    <li>
                        <a href="{{ route('backend.service.service-category.index') }}"> Categories </a>
                    </li>
                    <li>
                        <a href="{{ route('backend.expertise.index') }}"> Expertise </a>
                    </li>
                </ul>
            </li>

            <li class="menu">

                @php
                    $accessLogRoutes = [
                        'backend.user-access-log.index',
                        'backend.user-access-log.destroy',
                        'backend.user-activity-log.index',

                    ];
                @endphp

                <a href="#accessLogMenu" data-toggle="collapse"
                   aria-expanded="@if(route_exist_in_sidebar($accessLogRoutes)) true @else false @endif"
                   @if(route_exist_in_sidebar($accessLogRoutes)) data-active="true" @endif
                   class="dropdown-toggle @if(!route_exist_in_sidebar($accessLogRoutes))collapsed @endif">
                    <div class="">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                             stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                             class="feather feather-mouse-pointer">
                            <path d="M3 3l7.07 16.97 2.51-7.39 7.39-2.51L3 3z"></path>
                            <path d="M13 13l6 6"></path>
                        </svg>
                        <span>Accessibility Log</span>
                    </div>
                    <div>
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                             stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                             class="feather feather-chevron-right">
                            <polyline points="9 18 15 12 9 6"></polyline>
                        </svg>
                    </div>
                </a>
                <ul class="collapse submenu list-unstyled @if(route_exist_in_sidebar($accessLogRoutes)) show @endif"
                    id="accessLogMenu" data-parent="#accordionExample">
                    <li><a href="{{ route('backend.user-access-log.index') }}">AccessLog</a></li>
                    <li><a href="{{ route('backend.user-activity-log.index') }}">Activity Log</a></li>
                </ul>
            </li>

            @can(['add role','edit role'])
                <li class="menu">

                    @php
                        $permissionRoutes = [
                            'admin.role.create',
                            'admin.role.index',
                            'admin.role.show',
                             'admin.permission.create',
                            'admin.permission.index',
                            'admin.permission.show',
                        ];
                    @endphp

                    <a href="#permissionMenu" data-toggle="collapse"
                       aria-expanded="@if(route_exist_in_sidebar($permissionRoutes)) true @else false @endif"
                       @if(route_exist_in_sidebar($permissionRoutes)) data-active="true" @endif
                       class="dropdown-toggle @if(!route_exist_in_sidebar($permissionRoutes))collapsed @endif">
                        <div class="">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                 fill="none"
                                 stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                                 class="feather feather-shield">
                                <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"></path>
                            </svg>
                            <span>Role and Permission</span>
                        </div>
                        <div>
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                 fill="none"
                                 stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                                 class="feather feather-chevron-right">
                                <polyline points="9 18 15 12 9 6"></polyline>
                            </svg>
                        </div>
                    </a>
                    <ul class="collapse submenu list-unstyled @if(route_exist_in_sidebar($permissionRoutes)) show @endif"
                        id="permissionMenu" data-parent="#accordionExample">
                        <li><a href="{{ route('admin.role.index') }}">Role List</a></li>
                        <li><a href="{{ route('admin.role.create') }}"> Add Role</a></li>
                        <li><a href="{{ route('admin.permission.index') }}"> Permission List</a></li>
                        <li><a href="{{ route('admin.permission.create') }}"> Add Permission</a></li>
                    </ul>
                </li>
            @endcan


            <div class="ps__rail-x" style="left: 0px; bottom: 0px;">
                <div class="ps__thumb-x" tabindex="0" style="left: 0px; width: 0px;"></div>
            </div>
            <div class="ps__rail-y" style="top: 0px; height: 518px; right: -4px;">
                <div class="ps__thumb-y" tabindex="0" style="top: 0px; height: 239px;"></div>
            </div>
        </ul>

    </nav>

</div>
